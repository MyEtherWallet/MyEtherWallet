...
import {
  ...
  litecoinPath,
  type DerivationPath,
} from '@/modules/access/common/configs/configPaths'
...
export default class BitcoinWallet {
  ...
  async getWallet(index: number): Promise<BitcoinPrivateKeyWallet> {
    ...
    const hasLastTick =
      this.chainName === 'BITCOIN' || this.chainName === 'BITCOIN_TEST'

    const derived = hdkey.derive(
      `${this.basePath}/${index}${hasLastTick ? "'" : ''}`,
    )
    return new BitcoinPrivateKeyWallet(
      this.chainName,
      derived,
      WalletType.MNEMONIC,
    )
