...
export default class FromMnemonic {
  ...
  getMultipleAccounts(count, offset, callback) {
    try {
      this.getHDAddresses(
        this.currentWalletSet.start,
        this.currentWalletSet.limit,
        function(err, result) {
          if (err) {
            callback(err);
            return;
  ...
  signTransaction(rawTxData, callback) {
    try {
      ...
      const txData = {
        ...
        gas:
          rawTxData.gas || rawTxData.gasLimit
            ? rawTxData.gasLimit
  ...
  getAddress() {
    if (!this.wallet && this.HDWallet.wallets.length === 0)
      throw Error('no wallet present. wallet not have been decrypted');
    if (!this.wallet) {
  ...
  decryptWallet(options) {
    try {
      if (!bip39.validateMnemonic(options.manualmnemonic))
        throw new Error('Invalid Mnemonic Supplied');
      this.HDWallet.hdk = HDKey.fromMasterSeed(
        bip39.mnemonicToSeed(
          options.manualmnemonic.trim(),
  ...
  setHDAddresses(start = 0, limit = 5) {
    ...
    for (let i = start; i < start + limit; i++) {
      this.HDWallet.wallets.push(
        this.createWallet(
          this.HDWallet.hdk.derive(this.HDWallet.dPath + '/' + i)._privateKey
        )
      );
      this.HDWallet.wallets[this.HDWallet.wallets.length - 1].type =
        'addressOnly';
    ...
    if (this.HDWallet.hdk) {
      ...
      if (
        ...
        for (let i = 0; i < this.HDWallet.wallets.length; i++) {
          addressArray.push(
            utils.toChecksumAddress(this._getAddress(this.HDWallet.wallets[i]))
          );
        }
      } else {
        for (let i = 0; i < this.HDWallet.wallets.length; i++) {
