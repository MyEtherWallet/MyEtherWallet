...
export default class TrezorWallet extends HardwareWalletInterface {
      ...
      if (!this.decrypted) {
        this.isAvailableWait()
          .then(() => {
            ...
            console.log('collect', collect) // todo remove dev item
            resolve(collect)
          })
      } else {
        let collect = {}
  ...
  createWallet (priv, pub, path, hwType, hwTransport) {
    ...
    if (typeof priv !== 'undefined') {
      ...
      console.log('0x' + ethUtil.privateToAddress(wallet.privKey).toString('hex')) // todo remove dev item
    }
    console.log('PublicKey', '0x' + ethUtil.publicToAddress(pub, true).toString('hex')) // todo remove dev item
    wallet.pubKey = pub
    wallet.path = path
  ...
  setHDAddressesHWWallet (start, limit, ledger) {
    ...
    for (var i = start; i < start + limit; i++) {
      ...
      const tempWallet = this.createWallet(undefined, derivedKey.publicKey, this.HDWallet.dPath + '/' + i, this.identifier)
      this.addressToWalletMap[this._getAddressForWallet(tempWallet)] = tempWallet
      this.HDWallet.wallets.push(tempWallet)
      this.addressesToIndexMap[i] = this._getAddressForWallet(tempWallet)
  ...
  HWWalletCreate (publicKey, chainCode, walletType, path) {
    ...
    console.log(this.HDWallet) // todo remove dev item
  ...
  signTxTrezor (rawTx) {
    return new Promise((resolve, reject) => {
      ...
      var localCallback = function (result) {
        ...
        console.log('result.v', result.v) // todo remove dev item
        rawTx.v = '0x' + _this.decimalToHex(result.v)
        rawTx.r = '0x' + result.r
        rawTx.s = '0x' + result.s
        const eTx = new EthereumjsTx(rawTx)
        // rawTx.rawTx = JSON.stringify(rawTx)
        const rawTransaction = '0x' + eTx.serialize().toString('hex')
        // rawTx.isError = false
        console.log(rawTx) // todo remove dev item
        console.log(eTx.from.toString('hex')) // todo remove dev item
        console.log(eTx.validate())
        const toResolve = {
          rawTx: {
            nonce: rawTx.nonce,
            to: rawTx.to,
          ...
          messageHash: eTx.hash(), // figure out what exactly web3 is putting here
      ...
      console.log('wallet', this.wallet) // todo remove dev item
      console.log(this.HDWallet.dPath) // todo remove dev item
      TrezorConnect.signEthereumTx(
        ...
        (rawTx.data === '0x') ? '' : _this.getNakedAddress(rawTx.data),
