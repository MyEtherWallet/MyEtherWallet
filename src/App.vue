...
export default {
  ...
  mounted() {
    ...
    const web3 = store.get("network")
      ? new Web3(new Web3.providers.HttpProvider(store.get("network").url))
      : new Web3(
          new Web3.providers.HttpProvider(
            this.$store.state.Networks["ETH"][0].url
          )
        );
    const state = {
      web3: window.web3
        ? window.web3.setProvider(this.$store.state.Networks["ETH"][0].url)
        : web3,
      network:
