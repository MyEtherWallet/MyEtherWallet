...
export default {
  ...
  watch: {
    domain: async function (newVal, oldVal) {
      if (
        this.getActiveOverlay() === 'ManageRecordsOverlay' &&
        newVal &&
    ...
    errors: function (newVal) {
      if (this.getActiveOverlay() === 'ManageRecordsOverlay') {
        this.disabled = this.hasError(newVal);
      }
    }
  },
  methods: {
    ...mapMutations('unstoppable', ['setActiveOverlay']),
    ...
    async handleAddRecord(key) {
      const record = await this.resolution
        .getRecord(this.domain.name, keyToCryptoKey[key])
        .catch(() => '');
    ...
    async saveRecords() {
      this.loading = true;
      const resolverAddr =
        this.domain.resolver ||
        (await this.resolution.getResolver(this.domain.name));
      ...
      await resolverContract.methods
        .setMany(keys, values, this.domain.namehash)
