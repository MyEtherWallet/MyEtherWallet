...
export default {
  ...
  computed: {
    ...
    ipfsHash: {
      get() {
        return this.getManagedDomain().ipfsHash;
      },
      set(newHash) {
        return newHash;
      }
  ...
  watch: {
    domain: async function (newVal, oldVal) {
      if (this.getActiveOverlay() === 'UploadIpfsOverlay' && newVal) {
      ...
      try {
        const txObj = {
          ...
          data: resolverContract.methods
            .set('ipfs.html.value', hash, this.domain.namehash)
            .encodeABI(),
          value: 0
        };
