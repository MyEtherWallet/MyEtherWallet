...
3. In `TheDappNameLayout.vue`, add the following:
   ...
   | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | ------------------------------------------------------------------------------------------------ |
   | `bannerImage`   | `string`                                                                                                                                                        | Provides Banner Image to the dapp wrapper. <br> <pre lang="json">`default=@/assets/images/backgrounds/bg-dapps-center.png`</pre> |                                                                                                                                                              | `bannerText` | <pre lang="json">Object<br>{<br> title: i18n `//string` <br> subtext: i18n `//string`<br>}</pre> |
   | `tabItems`      | <pre lang="json">Object Array<br>{<br> title: i18n, `//string` <br> route: { <br> &nbsp; name: i18n `//string`, <br>&nbsp;},<br>&nbsp;id: 1 //number<br>}</pre> | Controls active dapp menu tab, leave it undefined if tabItems is undefined.                                                      | Provides Banner title and subtext <br> <pre lang="json">Example: <br> bannerText = { title: this.$t('ens.title'), subtext: this.$t('ens.dapp-desc') } </pre> |              |
   | `activeTab`     | `number`                                                                                                                                                        |                                                                                                                                  |
   | `validNetworks` | `Object / Array`                                                                                                                                                | Networks that the dapp is accessible on. All network objects can be found here: <br>`src/utils/networks/types`                   |
...
`title`, `subtitle` and `tag` will be displayed within the dapp center button. For the dapp center button image you can...
...
Important note about `store` management, if you look through the current stores we already have, you will see a key called `localStorage: boolean`. This key will define whether your state will be saved and synced with the `localStorage`.
