<template>
  <div>
      ...
      -->
        <v-card-text class="py-3 px-5 px-md-0">
          <slot name="dialogBody">
            <div>
              Proposed by: <b> {{ selectedProposal.proposer }} </b> &nbsp;
              Created at: <b> {{ selectedProposal.timestamp }} </b> &nbsp;
              Blocks until closure:
              <b> {{ selectedProposal.blocksUntilClosure }} </b>
            </div>
          </slot>
        </v-card-text>
        <v-card-text class="py-3 px-5 px-md-0">
          <slot name="dialogBody">
            <div>
              <b>Status: </b>:{{ selectedProposal.status }}, Snapshot:
              <b>{{ selectedProposal.proposalSnapshot }}</b>
            </div>
          </slot>
        </v-card-text>
        <v-card-text class="py-3 px-5 px-md-0">
          <slot name="dialogBody">
            <v-row class="pa-0">
              ...
              <v-col cols="6" sm="6" class="vote-btn-right">
                <mew-button
                  v-if="selectedProposal.status == 'Active'"
                  btn-style="outline"
                  btn-size="xlarge"
                  title="Vote on chain"
                ...
                <br />
                <div class="text-success mt-4 mb-1">
                  <b>Votes For: </b
                ...
                <div v-if="selectedProposal.calldatasParsed">
                ...
                <div v-if="tx" class="text-info">
                  <a :href="tx" target="_blank"> {{ msg }}: {{ tx }} </a>
                </div>
              </v-col>
            </v-row>
          </slot>
    ...
    <div v-if="!selectedProposal">
      ...
      <table v-if="proposals.length" class="dao-table mt-2">
        ...
        <tbody>
          <tr v-for="proposal in proposals" :key="proposal.proposalId">
            <td
...
import { getGovAddress } from '../../handlers/helpers/contracts';
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      const govAddress = getGovAddress(this.network.type.chainID);
