...
import BityCalls from './bity-calls';
import {
  bityStatuses,
  BityCurrencies,
  bityFiatCurrencies,
...
export default class BitySwap {
  ...
  async _getRateEstimate(fromCurrency, toCurrency, fromValue) {
    ...
    return await this.bityCalls.getEstimate(reqInfo);
  }
  ...
  async startSwap(swapDetails) {
    if (this.checkIfExit(swapDetails) && !swapDetails.bypass) {
      ...
      swapDetails.dataForInitialization = await this.bityCalls.createOrder(
        swapDetails
      );
  ...
  async buildOrder({
    ...
    if (this.maxCheck(fromCurrency, fromValue, toCurrency, toValue)) {
      ...
      return await this.bityCalls.openOrder(order);
    }
  }

  async getExitOrderDetails(detailsUrl) {
    return this.bityCalls.getCyptoToFiatOrderDetails(detailsUrl);
  }

  async sendSigned(signedDetails) {
    return this.bityCalls.sendSignedMessage(signedDetails);
  }
  ...
  static async getOrderStatusCrypto(errorHandler, noticeDetails) {
    if (typeof errorHandler !== 'function') {
      throw Error('First argument should be an error handling function');
    }
    let timeSinceOrder;

    try {
      const data = await new BityCalls(errorHandler).getStatus({
      ...
      if (timeSinceOrder) {
        if (timeSinceOrder < 100000) {
          errorHandler('Failed to retrieve Bity order status', 1, true);
        }
      }
    }
    return swapNotificationStatuses.PENDING;
  ...
  static async getOrderStatusFiat(errorHandler, noticeDetails) {
    if (typeof errorHandler !== 'function') {
      throw Error('First argument should be an error handling function');
    }
    try {
      const data = await new BityCalls(errorHandler).orderDetails({
