...
export default class Changelly {
  ...
  getFixedRate(fromCurrency, toCurrency, fromValue) {
    ...
    return new Promise(async resolve => {
      try {
        ...
        const changellyDetails = await this.changellyCalls.getFixRate(
          fromCurrency,
          toCurrency,
          fromValue,
          this.network
  ...
  async getMarketRate(fromCurrency, toCurrency, fromValue) {
    try {
      const changellyDetails = await Promise.all([
        this.changellyCalls.getMin(
          fromCurrency,
          toCurrency,
          fromValue,
          this.network
        ),
        this.changellyCalls.getRate(
          fromCurrency,
          toCurrency,
          fromValue,
          this.network
  ...
  async createFixedTransaction({
    ...
    return await this.changellyCalls.createFixTransaction(
      swapParams,
      this.network
    );
  }
  ...
  async createMarketTransaction({
    ...
    return await this.changellyCalls.createTransaction(
      swapParams,
      this.network
    );
  }
  ...
  async validateAddress(toCurrency, address) {
    return await this.changellyCalls.validateAddress(
      {
        currency: toCurrency,
        address: address
      },
  ...
  static async getOrderStatus(errorHandler, noticeDetails, network) {
    try {
      const status = await new ChangellyCalls(errorHandler).getStatus(
