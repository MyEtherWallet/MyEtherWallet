<template>
  <div class="grid grid-cols-1">
    <div
      class="border border-line-strong rounded-2xl p-1 xs:p-4 flex flex-col gap-1"
    >
      <div v-for="i in 5" :key="i">
        <button
          v-if="!isLoading && walletList[i - 1]"
          :class="[
            walletList[i - 1].index === model ? 'bg-page' : 'hoverNoBG',
            'flex px-2 xs:px-4 py-2 items-center gap-1 xs:gap-2 xs:gap-5 w-full rounded-2xl  min-h-12',
          ]"
          @click="model = walletList[i - 1].index"
          v-ripple
        >
          <div class="hidden xs:block text-xs text-fg-subtle min-w-5">
          ...
          <div
            :class="[
              {
                'bg-brand': walletList[i - 1].index === model,
              },
              'border border-brand border-2 w-5 h-5 ml-auto rounded-full flex items-center justify-center transition-colors',
            ]"
          >
            <check-icon
              v-if="walletList[i - 1].index === model"
              class="text-fg-on-fill w-3 h-3"
            />
          </div>
        </button>
        <div
          v-else
          class="min-h-12 animate-pulse bg-page my-2 rounded-2xl"
        ></div>
      </div>
    </div>
