...
const getHexTxObject = (tx: Transaction) => {
  return {
    to: tx.to ? tx.to.toString() : undefined,
    ...
    gasPrice: tx.gasPrice ? bigIntToHex(tx.gasPrice) : undefined,
  }
}
  ...
  return {
    ...
    tx: {
      ...
      gasPrice: tx.gasPrice
        ...
        : bigIntToHex(tx.maxFeePerGas as bigint),
      gas: tx.gasLimit
        ? bigIntToHex(tx.gasLimit)
        : bigIntToHex(tx.gas as bigint),
      to: tx.to ? tx.to : '',
      value: bigIntToHex(tx.value),
...
const eip1559Params = (
  ...
  const fees = {
    maxPriorityFeePerGas:
      tip < getMinPriorityFee()
        ? bigIntToHex(getMinPriorityFee())
        : tip > toBigInt(feeMarket.maxPriorityFeePerGas)
          ? bigIntToHex(toBigInt(feeMarket.maxPriorityFeePerGas))
          : bigIntToHex(tip),
...
const getWalletFromPrivKeyFile = (
  ...
  if ((json as EthSaleKeystore).encseed != null) {
    return Wallet.fromEthSale(json as EthSaleKeystore, password)
  } else if ((json as V3Keystore).crypto != null) {
    return Wallet.fromV3(json as V3Keystore, password, true)
  } else if ((json as EtherWalletOptions).hash != null) {
...
const unlockKeystore = async (
  file: EthSaleKeystore | V3Keystore | MEWKeystore | EtherWalletOptions,
  pw: string,
) => {
  const newFile: Record<string, string> = {}
  Object.keys(file).forEach((key: string) => {
    newFile[key.toLowerCase()] = file[key as keyof typeof file]
  })
