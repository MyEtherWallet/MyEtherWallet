...
export default class WalletInterface {
  keystore: {
    file: EthSaleKeystore | V3Keystore | MEWKeystore | null
    name: string
  } | null
  identifier: string
  isPub: boolean
  ...
  constructor(
    ...
    identifier: string,
    keystore?: {
      file: EthSaleKeystore | V3Keystore | MEWKeystore
      name: string
    },
  ) {
    ...
    if (!isPub) {
      const _privateKey = ArrayBuffer.isView(key)
        ? key
        : hexToBytes(key as string)
      if (
        _privateKey.length !== 32 ||
        !isValidPrivate(_privateKey as Uint8Array)
      ) {
        throw new Error(
      ...
      const _publicKey = ArrayBuffer.isView(key)
        ? key
        : hexToBytes(key as string)
      if (_publicKey.length !== 20) {
        try {
          if (!isValidPublic(_publicKey as Uint8Array, true))
  ...
  signTransaction(
    ...
    signer: (txParams: Transaction) => string,
  ) {
    ...
    if (this.isPubOnly && typeof signer !== 'function')
      throw new Error('Public key wallet needs a signer function')
    if (!this.isPubOnly) {
      return new Promise(resolve => {
        ...
        if (isEIP1559SupportedNetwork.value) {
          ...
          const _txParams = Object.assign(
            eip1559Params(txParams.gasPrice as bigint, feeMarket),
            txParams,
            { gasPrice: null },
          )
        ...
        const signedChainId = signedTxToJson.chainId
          ? parseInt(signedTxToJson.chainId.toString())
          : calculateChainIdFromV(signedTxToJson.v as string)
        if (chainId !== BigInt(signedChainId)) {
          throw new Error(
            `Invalid networkdId signature returned. Expected: ${chainId}, Got: ${signedChainId}`,
          )
        }
    ...
    return new Promise((resolve, reject) => {
      if (!this.isPubOnly) {
        const msgHash = hashPersonalMessage(
          new Uint8Array(Buffer.from(msg, 'utf-8')),
        )
