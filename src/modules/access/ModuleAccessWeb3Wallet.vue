      ...
      <app-sheet>
        ...
        >
          ...
          <template v-if="web3ConnectionError">
            <p class="text-s-17 font-semibold text-center mb-2 text-text-error">
              {{ $t('web3_wallet.connection_failed') }}
            </p>
            <p class="text-text-subtle text-s-14 text-center mb-6">
              {{ web3ConnectionError }}
            </p>
            <app-base-button
              :disabled="!canRetry"
          ...
          <template v-else>
            ...
            <!-- Confirm Message -->
            <p class="text-text-subtle text-center mb-6">
              {{ $t('web3_wallet.confirm_in_extension') }}
            </p>
            <!-- Loading Spinner -->
            <svg
              aria-hidden="true"
              class="w-8 h-8 text-text-brand animate-spin fill-white mb-6"
              viewBox="0 0 100 101"
              width="32"
              height="32"
              fill="none"
