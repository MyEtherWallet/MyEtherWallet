      ...
      if (!mywallet.isConnected) {
        ...
        mywallet.ws.onmessage = function incoming(data) {
          ...
          try {
            ...
            errorHandler({
              message: 'Satochip: error while connecting to Satochip-Bridge'
            });
          }
        };
  ...
  getAccount(idx) {
    ...
    const txSigner = async tx => {
      ...
      const tx_info = {
        ...
        tx_hash_true: tx_hash_true,
        tx_hash_false: tx_hash_false,
        chainId: networkId,
        address: address
      }; // debugsatochip
      ...
      const signedChainId = calculateChainIdFromV(
        getBufferFromHex(result.v.toString(16))
      );
