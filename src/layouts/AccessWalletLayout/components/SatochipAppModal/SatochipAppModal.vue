<template>
  <div>
    <b-modal
      ref="satochipApp"
      :title="$t('accessWallet.satochip.modal.title')"
      hide-footer
      ...
      @hidden="reset"
    >
      <div class="satochip-app-selection-container">
        <h4>{{ $t('accessWallet.satochip.modal.text') }}</h4>
        <div class="satochip-app-info">
...
<script>
import apps from '@/wallets/hardware/satochip/appPaths.js';
import cust from '@/assets/images/icons/network.svg';
import { Toast, pathHelpers } from '@/helpers';
import { SatochipWallet } from '@/wallets';
...
export default {
  ...
  methods: {
    ...
    next() {
      this.$refs.satochipApp.hide();
      SatochipWallet(this.selectedPath.path)
        .then(_newWallet => {
