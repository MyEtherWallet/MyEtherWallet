...
export interface FetchMewAPIResponse {
  ...
  useMEWWalletFetch: ReturnType<typeof createFetch>
}

/** A single data point: [ISO timestamp, balance as a decimal string] */
export type BalancePoint = [timestamp: string, balance: string]

export interface PortfolioBalanceHistoryResponse {
  chains: string[]
...
export const useFetchMewWalletApi = (
  _poll: number = 0,
   ...
   */
  const useMEWWalletFetch = createFetch({
    baseUrl: Configs.MEW_PURCHASE_BASE_URL,
    options: {
      beforeFetch: ctx => {
        url.value = ctx.url
      },
