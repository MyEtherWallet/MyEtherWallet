...
| `modules/trade/composables/`         | `useApprovalFee`, `useMarketStatusDisplay`, `useTrade`, `useTradeBreakdown`, `useTradeExecution`, `useTradeForm`, `useTradeModule`, `useTradeQuote`, `useTradeTokens`, `useTradeValidation`                                                                                                                                                                                                                                                                                                                 | None. The pure helpers live in `modules/trade/common/` (`marketDisplay`, `tradeSession`, error classifiers) and `src/utils/tradeExplorerLink.ts`. `useMarketStatus` was deleted: it was a stateless pass-through over `stores/marketStatusStore.ts` (with a dead `onMarketOpen` option), so call sites read the store directly and refcount its polling via `acquire`/`release`. The quote/validation option bags still write to the caller-owned form refs (`toAmount`, `generalError`, `isLoadingQuote`, the pair/minimum flags) — the shared form record breaks a genuine circular dependency between `useTradeValidation` (which needs the quote flags) and `useTradeQuote` (which needs `hasPreQuoteError`), so `useTradeQuote` is documented as the sole writer of the quote-owned flags via `runQuote`/`resetQuote`. |
...
rather than moved. `onPerpsAuthReset` registrations were _not_ folded into token
