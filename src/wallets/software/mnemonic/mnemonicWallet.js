...
export default class MnemonicWallet extends HardwareWalletInterface {
  ...
  decryptWallet (options) {
    try {
      ...
      this.hdk = HDKey.fromMasterSeed(bip39.mnemonicToSeed(options.mnemonicPhrase.trim(), options.mnemonicPassword))
      this.setHDAddresses()
    } catch (e) {
      throw e
    }
  ...
  setHDAddresses (start = 0, limit = 5) { // TODO: Move to a worker thread
    this.walletsRetrieved = []
