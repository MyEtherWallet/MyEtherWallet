<template>
  <div class="flex flex-col gap-4 p-4 h-full min-h-0">
    ...
    <div class="flex items-center gap-1 w-full shrink-0">
      ...
      <span
        class="flex-1 text-center text-s-16 font-semibold text-black tracking-[-0.32px]"
      >
    ...
    <div class="shrink-0 flex flex-col gap-4">
      ...
      <div
        class="flex items-center gap-2.5 h-10 px-2.5 rounded-[24px] bg-background-default-hover w-full"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 20 20"
        ...
        <input
          ...
          class="bg-transparent outline-none border-0 w-full text-s-14 text-black placeholder:text-text-muted"
        />
      </div>
    </div>
    ...
    <div class="flex-1 min-h-0 overflow-y-auto flex flex-col gap-1">
      ...
      <button
        ...
        :class="[
          'flex items-center gap-3 p-4 rounded-[22px] w-full cursor-pointer transition-colors hover:bg-background-default',
          chain.name === chainsStore.selectedChain?.name
            ? 'bg-background-default rounded-[16px]'
            : '',
        ]"
        @click="apply(chain)"
      >
        ...
        <div
          v-else
          class="w-7 h-7 rounded-full bg-background-default-hover shrink-0"
        />
        <span
          class="flex-1 text-left text-s-14 font-semibold text-black tracking-[-0.28px]"
        >
          {{ chain.nameLong }}
        </span>
      ...
      <template v-if="incompatible.length">
        ...
        >
          ...
          <div
            v-else
            class="w-7 h-7 rounded-full bg-background-default-hover shrink-0 opacity-50"
          />
          <span
            class="text-s-14 font-semibold text-text-placeholder tracking-[-0.28px]"
          >
            {{ chain.nameLong }}
          </span>
        </div>
