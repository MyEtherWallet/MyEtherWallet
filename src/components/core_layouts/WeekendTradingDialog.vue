    ...
    <template #content>
      <div class="p-6 h-full flex flex-col justify-between">
        ...
        <div class="flex flex-col gap-4">
          <div class="flex items-center">
            <img
              ...
              :src="icon"
              alt=""
              class="w-10 h-10 rounded-full border-[1.5px] border-surface bg-surface object-cover"
              :class="i > 0 ? '-ml-2' : ''"
            />
          </div>
        ...
        <div class="flex flex-col gap-5 mt-8 xs:mt-0">
          <p class="text-s-16 leading-[22px] text-fg-subtle">
            {{
          ...
          <div class="flex gap-2">
            <button
              class="flex-1 h-12 rounded-24 bg-brand text-fg-on-fill text-s-16 font-semibold"
              @click="onTradeNow"
            >
              {{ $t('trade.weekend.trade_now') }}
            </button>
            <button
              class="flex-1 h-12 rounded-24 bg-surface-strong text-fg text-s-16 font-semibold"
              @click="onGotIt"
            >
              {{ $t('common.got_it') }}
            </button>
