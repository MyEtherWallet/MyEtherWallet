      ...
      <div class="modal-contents">
        ...
        >
          ...
          <div class="button-block">
            ...
            <!-- <standard-button :options="buttonExport" @click.native="exportConfig"/> -->
          </div>
        </full-width-dropdown>
      </div>
    </b-modal>
...
export default {
  ...
  data() {
    return {
      ...
      gasPriceInputs: {
        slow: {
          gwei: new BigNumber(
            utils.fromWei(`${this.gasPrice / 2}`, 'gwei')
          ).toFixed(),
          eth: new BigNumber(
            utils.fromWei(`${this.gasPrice / 2}`, 'ether')
          ).toFixed()
        },
        regular: {
          gwei: new BigNumber(
        ...
        fast: {
          gwei: new BigNumber(
            utils.fromWei(`${this.gasPrice * 1.25}`, 'gwei')
          ).toFixed(),
          eth: new BigNumber(
            utils.fromWei(`${this.gasPrice * 1.25}`, 'ether')
          ).toFixed()
        }
      },
      customGas: 0,
  ...
  methods: {
    ...
    async getEthPrice() {
      ...
      this.ethPrice = rates.data[1027].quotes.USD.price;
    }
  }
};
</script>
