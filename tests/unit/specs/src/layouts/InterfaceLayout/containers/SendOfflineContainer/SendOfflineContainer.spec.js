...
describe('SendOfflineContainer.vue', () => {
  ...
  xit('[FAILING] should render correct nonce data', () => {
    expect(
      wrapper.vm.$el.querySelectorAll('.gas-amount input')[2].value
    ).toEqual(String(wrapper.vm.$data.nonce));
  });

  xit('[FAILING] should render correct gasLimit data', () => {
    expect(
      wrapper.vm.$el.querySelectorAll('.gas-amount input')[3].value
  ...
  xit('[FAILING] should render correct rawTx data', () => {
    ...
    expect(
      wrapper.vm.$el.querySelector('.gas-amount textarea').textContent
    ).toEqual('');
  });

  xit('[FAILING] should render correct currentPage data', () => {
    const currentPage = wrapper.vm.$el.getElementsByClassName(
      wrapper.vm.$data.currentPage
    )[0];
  ...
  describe('SendOfflineContainer.vue Methods', () => {
    xit('[FAILING] should change currentPage when click button', () => {
      ...
      for (let i = 0; i < processElements.length; i++) {
        const processElement = processElements.at(i);
