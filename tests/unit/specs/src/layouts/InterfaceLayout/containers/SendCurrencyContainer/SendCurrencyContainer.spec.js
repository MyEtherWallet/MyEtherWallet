...
describe('SendCurrencyContainer.vue', () => {
  ...
  it('should render correct amount data', () => {
    expect(wrapper.vm.$el.querySelector('.amount-number input').value).toEqual(String(wrapper.vm.$data.amount));
  });

  it('should render correct "data" data', () => {
    wrapper.setData({ advancedExpend: true});
    expect(wrapper.vm.$el.querySelector('.user-input input').value).toEqual(wrapper.vm.$data.data);
  });

  it('should render correct gasLimit data', () => {
    wrapper.setData({ advancedExpend: true });
    expect(wrapper.vm.$el.querySelectorAll('.user-input input')[1].value).toEqual(String(wrapper.vm.$data.gasLimit));
  });
