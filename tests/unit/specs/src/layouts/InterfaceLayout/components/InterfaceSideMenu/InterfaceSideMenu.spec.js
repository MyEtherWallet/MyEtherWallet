...
describe('InterfaceSideMenu.vue', () => {
  ...
  xit('[FAILING] should render correct showSend', () => {
    wrapper.setData({ showSend: true });
    expect(
      wrapper.vm.$el
        .querySelectorAll('.menu-group-title i')[0]
        .className.indexOf('fa-angle-up')
    ).toBeGreaterThan(-1);
  });

  xit('[FAILING] should render correct showContract', () => {
    wrapper.setData({ showContract: true });
    expect(
      wrapper.vm.$el
        .querySelectorAll('.menu-group-title i')[3]
        .className.indexOf('fa-angle-up')
    ).toBeGreaterThan(-1);
  });

  xit('[FAILING] should render correct showMessage', () => {
    wrapper.setData({ showMessage: true });
    expect(
      wrapper.vm.$el
        .querySelectorAll('.menu-group-title i')[4]
        .className.indexOf('fa-angle-up')
    ).toBeGreaterThan(-1);
  });

  xit('[FAILING] should render correct tabData', () => {
    ...
    for (let i = 0; i < groupElements.length; i++) {
      expect(groupElements[i].querySelector('p').textContent).toEqual(
        wrapper.vm.$data.tabData[i].title
      );
      if (wrapper.vm.$data.tabData[i].caret) {
        for (let j = 0; j < menuElement.querySelectorAll('li').length; j++) {
          const liText = menuElement
            .querySelectorAll('li')
            // eslint-disable-next-line no-unexpected-multiline
            [j].textContent.trim();

          if (wrapper.vm.$data.tabData[i].contents[j] !== undefined) {
            const itemTitle = menuElement
              .querySelectorAll('li')
              // eslint-disable-next-line no-unexpected-multiline
              [j].textContent.trim();
  ...
  describe('InterfaceSideMenu.vue Methods', () => {
    xit('[FAILING] should toggle showSend when click button', () => {
      wrapper
        .findAll('.menu-group-title')
        .at(0)
        .trigger('click');
    ...
    xit('[FAILING] should toggle showContract when click button', () => {
      wrapper
        .findAll('.menu-group-title')
        .at(3)
        .trigger('click');
    ...
    xit('[FAILING] should toggle showMessage when click button', () => {
      wrapper
        .findAll('.menu-group-title')
        .at(4)
        .trigger('click');
    ...
    xit('[FAILING] should switch tab when button click', () => {
      const itemClickElements = wrapper
        .findAll('ul')
        .at(0)
        .findAll('ul li');
      for (let i = 0; i < itemClickElements.length; i++) {
